...
@section('content')
    ...
    <div class="mx-auto flex max-w-[1200px] flex-col gap-6">
        ...
        <section class="rounded-xl border border-[#c7c6ca] bg-white p-6">
            <div class="mb-6 flex items-center justify-between"><h2 class="font-['Plus_Jakarta_Sans'] text-xl font-semibold">Statistik Performa</h2><span class="rounded-lg border border-[#c7c6ca] bg-[#f9f9f9] px-4 py-2 text-sm font-semibold text-[#46464a]">7 Hari Terakhir</span></div>
            <div class="relative flex h-64 items-end border-b border-[#c7c6ca] pb-7 pl-8">
                ...
                <div class="flex h-full w-full items-end justify-between gap-3 md:gap-6">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <section class="overflow-hidden rounded-xl border border-[#c7c6ca] bg-white"><header class="flex items-center justify-between border-b border-[#c7c6ca] px-4 py-4"><h2 class="flex items-center gap-2 text-sm font-semibold"><span class="h-2 w-2 animate-pulse rounded-full bg-[#795902]"></span>Antrean Aktif</h2><a href="{{ route('barber.queue') }}" class="text-xs font-medium text-[#46464a] hover:text-black">Lihat Semua</a></header><div>
                @forelse($servingBookings->take(3) as $booking)
                    <div class="flex items-center justify-between border-b border-[#e2e2e2] p-4 last:border-0"><div class="flex items-center gap-4"><div class="grid h-10 w-10 place-items-center rounded {{ $booking->status === 'serving' ? 'bg-[#010102] text-white' : 'bg-[#eeeeee] text-[#46464a]' }} text-sm font-bold">{{ $booking->queue_number }}</div><div><p class="text-sm font-semibold">{{ $booking->customer->name }}</p><p class="text-xs text-[#46464a]">{{ $booking->service->service_name }}</p></div></div><div class="text-right"><span @class(['inline-block rounded px-2 py-1 text-[10px] font-semibold', 'bg-[#fdd275]/30 text-[#795902]' => $booking->status === 'serving', 'bg-[#eeeeee] text-[#46464a]' => $booking->status !== 'serving'])>{{ $booking->status === 'serving' ? 'Sedang Dilayani' : ucfirst($booking->status) }}</span><p class="mt-1 text-[10px] text-[#46464a]">{{ $booking->slot_time?->format('H:i') ?? '-' }} WIB</p></div></div>
